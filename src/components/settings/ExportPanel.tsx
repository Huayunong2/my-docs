...
export default function ExportPanel() {
  ...
  const exportFull = async () => {
    ...
    try {
      ...
      setMsg(`✅ 完整备份已下载（${data.articles?.length||0} 篇记录 + ${data.reviews?.length||0} 篇复盘）`); setTone("good");
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.articles && data.reviews) {
        ...
        setMsg(`✅ 导入 ${result.imported_articles} 篇记录 + ${result.imported_reviews} 篇复盘`);
        setTone("good");
      } else {
        // Plain article array
    ...
    <div className="space-y-4 max-w-3xl">
      <Card>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <PrimaryBtn onClick={exportZip} disabled={loading}>📝 下载 Markdown ZIP</PrimaryBtn>
          <SecondaryBtn onClick={exportJson} disabled={loading}>📦 导出 JSON 到服务器</SecondaryBtn>
          <PrimaryBtn onClick={exportFull} disabled={loading}>💾 下载完整备份</PrimaryBtn>
          <SecondaryBtn onClick={() => fileRef.current?.click()} disabled={loading}>📥 导入完整备份</SecondaryBtn>
        </div>
      </Card>
