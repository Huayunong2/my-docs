...
export default function HistoryPage({
  ...
  const closeDetail = useCallback(() => {
    ...
    closeTimerRef.current = window.setTimeout(() => {
      ...
      if (
        ...
        document.contains(detailTriggerRef.current)
      ) {
  ...
  const retryDetail = useCallback(() => {
    if (selectedId)
      void openDetail(selectedId, detailTriggerRef.current || undefined);
  ...
  const historyReturnTo = useCallback(() => {
    ...
    if (view === "month" && selectedMonth)
      search.set("historyMonth", selectedMonth);
  ...
  const handleDelete = useCallback(
    async (
      ...
      try {
        ...
        setHasMore(
          (current) => current || page * PAGE_SIZE < Math.max(0, total - 1),
        );
    ...
    async (article: ArticleSummary) => {
      try {
        ...
        if (viewRef.current === "trash")
          void loadPage(pageRef.current, "trash", false);
  ...
  const locateDate = useCallback(
    async (date: string) => {
      ...
      try {
        ...
        toast.error(`查找 ${date} 失败：${api.getErrorMessage(lookupError)}`, {
          duration: 3600,
        });
      } finally {
  ...
  useEffect(() => {
    ...
    const update = () =>
      setMobileDetailOpen(Boolean(selectedId) && media.matches);
    media.addEventListener("change", update);
    return () => media.removeEventListener("change", update);
  }, [selectedId]);
  return (
    <div className="ft-page ft-history" data-detail={!!selectedId}>
      <WorkspaceHeader
        icon={CalendarDays}
        title={view === "trash" ? "记录回收站" : "记录"}
        description=""
        actions={
          <>
            <Link
      ...
      <div className="history-toolbar shrink-0 px-4 sm:px-6 md:px-8 xl:px-6">
        <div className="history-toolbar-inner flex w-full min-w-0 flex-wrap items-center justify-between gap-3">
          ...
          >
            <TabsList
              aria-label="记录视图"
              className="history-tabs w-full sm:w-auto"
            >
              <TabsTrigger
                value="timeline"
                className="history-tab min-w-0 flex-1 sm:flex-none"
              >
                <List size={14} aria-hidden="true" /> 时间线
              </TabsTrigger>
              <TabsTrigger
                value="month"
                className="history-tab min-w-0 flex-1 sm:flex-none"
              >
                <CalendarRange size={14} aria-hidden="true" /> 按月份
              </TabsTrigger>
              <TabsTrigger
                value="trash"
                className="history-tab min-w-0 flex-1 sm:flex-none"
              >
                <Trash2 size={14} aria-hidden="true" /> 回收站
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <div className="history-filter-region flex min-w-0 items-center gap-3">
            {view === "timeline" && (
              <DatePickerPopover
                ...
                onChange={locateDate}
                label="定位日期"
                className="history-date-picker w-full sm:w-[178px]"
              />
            )}
            {view === "month" && (
              <label className="history-filter-field block min-w-0 sm:w-[210px]">
                ...
                {months.length > 0 ? (
                  ...
                  >
                    <SelectTrigger
                      className="history-select-trigger"
                      aria-label="选择月份"
                    >
                    ...
                    <SelectContent
                      align="end"
                      className="history-select-content"
                    >
                      {months.map((month) => {
                  ...
                  <div className="history-select-empty" role="status">
                    {loading ? "加载月份…" : "暂无月份"}
                  </div>
                )}
              </label>
            ...
            {view === "trash" && (
              <span className="ft-caption">{total} 条可恢复记录</span>
            )}
            {dateLookupLoading && (
              <span className="history-lookup-status" role="status">
                正在查找…
              </span>
            )}
          </div>
      ...
      <div className="history-body flex min-h-0 flex-1 flex-col xl:flex-row">
        <section
          className="history-list-pane flex min-h-0 min-w-0 flex-1 flex-col"
          aria-label="每日记录列表"
        >
          <div className="history-list-scroll min-h-0 flex-1 overflow-y-auto px-4 pb-24 pt-5 sm:px-6 sm:pt-6 md:px-8 xl:px-6">
            <div className="history-list-header">
              ...
              <span className="history-list-count">
                {loading && articles.length === 0
                  ? "…"
                  : `${articles.length}${articles.length < total ? ` / ${total}` : ""}`}{" "}
                条
              </span>
            </div>
            ...
            <div className="history-timeline">
              ...
              <AnimatePresence initial={false}>
                {groupedArticles.map((group) => (
                  <section
                    key={group.key}
                    className={`history-group ${group.key === "today" ? "is-today" : ""}`}
                  >
                    <div className="history-group-rail" aria-hidden="true">
                      <span
                        className={`history-group-marker ${group.key === "today" ? "is-today" : ""}`}
                      />
                    </div>
                        ...
                        {group.items.map((article) =>
                          view === "trash" ? (
                            ...
                            <HistoryCard
                              ...
                              onEdit={(event) =>
                                handleEdit(article.date, event)
                              }
                              onDelete={(event) =>
                                void handleDelete(article, event)
                              }
                            />
                          ),
                        )}
            ...
            {hasMore && (
              <button
                type="button"
                onClick={() => void loadPage(page + 1, view)}
                disabled={loading}
                className="history-load-more"
              >
                {loading
                  ? "加载中…"
                  : `加载更多（还剩 ${Math.max(0, total - articles.length)} 条）`}
              </button>
            )}
          </div>
        </section>
        ...
        {selectedId && (
          <section
            className="history-detail-pane hidden min-h-0 min-w-0 flex-1 xl:flex"
            aria-label="记录详情"
          >
            <AnimatePresence mode="wait" initial={false}>
              {selectedId && detailStatus === "loading" && (
                <DetailLoading key="loading" />
              )}
...
function HistoryListLoading({ label }: { label: string }) {
  return (
    <div
      className="history-list-loading"
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
    ...
    <div className="history-empty-state">
      ...
      {!isTrash && (
        <button
          type="button"
          onClick={onWrite}
          className="ui-button-primary history-empty-action"
        >
          <PenLine size={14} /> 去记录
        </button>
      )}
...
function DetailLoading() {
  return (
    <div
      className="mx-auto flex min-h-[320px] w-full max-w-2xl flex-col gap-4 py-6"
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
  ...
  return (
    <div className="mx-auto flex min-h-[320px] w-full max-w-xl flex-col justify-center gap-4">
      ...
      <button
        type="button"
        onClick={onClose}
        className="ui-button-secondary min-h-11 self-start sm:min-h-9"
      >
        关闭详情
      </button>
    </div>
    ...
    >
      ...
      >
        <div className="history-card-topline">
          <time className="history-card-date" dateTime={article.date}>
            {formatShortDate(article.date)}
          </time>
          {article.mood && (
            <span className="history-card-mood" aria-label="当天心情">
              {article.mood}
            </span>
          )}
        </div>
        <h3
          className={`history-card-title break-words ${article.title?.trim() ? "" : "is-empty"}`}
        >
          {article.title || "(无标题)"}
        </h3>
        <p
          className={`history-card-preview line-clamp-2 break-words ${preview ? "" : "is-empty"}`}
        >
          {preview || "暂无正文摘要"}
        </p>
      </button>
      <div
        className={`history-card-footer ${spaces.length > 0 || tags.length > 0 ? "" : "is-actions-only"}`}
      >
        <div
          className="history-card-context flex min-w-0 flex-wrap gap-1.5"
          aria-label="记录所属空间和主题"
        >
          {spaces.slice(0, 2).map((space) => (
            <span
              key={space}
              className="history-card-chip history-card-chip-space"
            >
          ...
          {tags.slice(0, 3).map((tag) => (
            <span key={tag} className="history-card-chip">
              #{tag}
            </span>
          ))}
          {(spaces.length > 2 || tags.length > 3) && (
            <span className="history-card-more">
              +{Math.max(0, spaces.length - 2) + Math.max(0, tags.length - 3)}
            </span>
          )}
        </div>
  ...
  return (
    <div className="history-card-actions flex shrink-0 items-center gap-1">
      <button
        ...
        aria-label={`编辑 ${date} 的记录`}
      >
        <PenLine size={13} aria-hidden="true" /> 编辑
      </button>
      <DropdownMenu>
    ...
    >
      <div className="min-w-0">
        <div className="history-trash-meta flex flex-wrap items-center gap-2">
          <span className="history-trash-status">
            <Trash2 size={12} aria-hidden="true" /> 已移入回收站
          </span>
          <time className="history-card-date" dateTime={article.date}>
            {formatShortDate(article.date)}
          </time>
        </div>
        <h3 className="history-trash-title break-words">
          {article.title || "(无标题)"}
        </h3>
        <p className="history-trash-preview line-clamp-2 break-words">
          {cleanPreview(article.preview) || "暂无正文摘要"}
        </p>
      </div>
      <button
        type="button"
        onClick={onRestore}
        className="history-restore-button shrink-0"
        aria-label={`恢复 ${article.date} 的记录`}
      >
...
function groupArticlesByTime(articles: ArticleSummary[]) {
  ...
  const groups = new Map<
    string,
    { key: string; label: string; items: ArticleSummary[] }
  >();
