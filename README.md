...
- 间隔重复复习：按遗忘曲线调度已沉淀卡片，忘记/困难/记得/轻松四档评分，支持键盘快捷键。
- 复用追踪：记录卡片被打开使用的次数与最近使用时间，可筛选"从未使用"的冷卡回炉。
- 场景化搜索：全文搜索页可同时检索文章与知识卡片，命中卡片一键跳转打开。
- SQLite 快照备份、下载、删除。
...
- 设置页创建的手工快照保留 10 份；
- 超过 24 小时的已知临时快照、运维临时目录和中断部署 staging 会自动清理。
